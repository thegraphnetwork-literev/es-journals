...
if makim scheduler.download-rxivr --server ${server} --begin ${latest_date} --end ${current_date} --target ${path_root}/data/rxivx/${server}/downloaded/ ; then
    ...
    if python ${path_root}/scripts/merge_arxiv_data.py "${path_root}/data/rxivx/${server}/final/${server}_full_data.json" "data/rxivx/${server}/downloaded/${output_filename}"; then
